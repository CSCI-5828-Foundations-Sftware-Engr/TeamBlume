...
export const scrapeAmazon = async () => {
  try {
    const result = await axios.get(
      "https://www.amazon.com/s?k=keyboard&rh=n%3A172282%2Cp_89%3AKeychron&dc&ds=v1%3AFUA1yqkcxwtsK0aIwNxdhCxqMEBa38xMZ36NHSDziUg&crid=1XT8C0G6X00TW&qid=1680129742&rnid=2528832011&sprefix=%2Caps%2C94&ref=sr_nr_p_89_1",
      {
        headers: {
...
export const scrapeBestBuy = async () => {
  try {
    const result = await axios.get(
      "https://www.bestbuy.com/site/searchpage.jsp?id=pcat17071&qp=brand_facet%3DBrand~Logitech&st=mouse",
      {
        headers: {
          "Accept-enconding": "application/gzip",
          "User-Agent": "axios 1.3.4",
...
const amazon = new Queue("amazon");
const bestbuy = new Queue("bestbuy");
...
const main = async () => {
    ...
    await amazon.add({ amazonProducts: amazonRes },
        repeat = {
            cron: '*/1 * * * *',
            limit:1
        }
    ...
    await bestbuy.add({ bestbuyProducts: bestbuyRes },
        repeat = {
            cron: '*/1 * * * *',
            limit:1
        }
